...
// src/llm.ts
var LLMProcessor = class {
  constructor(config) {
    this.config = config;
  }
  async processTickets(tickets) {
  ...
  async processWithClaude(tickets) {
    var _a;
    ...
    if (!response.ok) {
      ...
      return {
        success: false,
        message: `Claude API error: ${((_a = errorData.error) == null ? void 0 : _a.message) || response.statusText}`
      };
    }
  ...
  async processWithOllama(tickets) {
  ...
  async processWithCopilot(tickets) {
    return {
      success: false,
      message: "GitHub Copilot integration is not implemented yet"
    };
  }
  preparePromptForClaude(tickets) {
...
"
  ...
  preparePromptForOllama(tickets) {
    return `You are an AI assistant helping with software development tasks.
...
"
  ...
  parseClaudeResponse(content, originalTickets) {
    try {
      ...
      const updatedTickets = originalTickets.map((ticket) => {
        var _a;
        const update = (_a = parsed.updatedTickets) == null ? void 0 : _a.find((t) => t.id === ticket.id);
        if (update) {
          return {
            ...ticket,
  ...
  parseOllamaResponse(content, originalTickets) {
    try {
      const jsonMatch = content.match(/```json\n([\s\S]*?)\n```/) || content.match(/\{[\s\S]*\}/);
      if (!jsonMatch) {
        return {
...
var Kanban2YAMLPlugin = class extends import_obsidian.Plugin {
  constructor() {
    ...
    this.llmProcessor = new LLMProcessor(DEFAULT_SETTINGS.llmIntegration);
  }
  async onload() {
  ...
  async saveTicketsToYAML(tickets) {
